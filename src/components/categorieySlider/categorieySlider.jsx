...
import styles from './categorieySlider.module.css'; 
...
export default function CategorieySlider() {
  ...
  return (
    <>
      ...
      {isLoading ? (
        <div className={`w-100 d-flex justify-content-center align-items-center position-fixed start-0 end-0 top-0 bottom-0 bg-white z-3 ${styles.loader}`}>
      ...
      <div className={`container py-4 ${styles.container}`}>
        <h2 className={`pt-4 ${styles.heading}`}>Shop Popular Categories</h2>
        {cat ? (
          <Slider {...settings}>
            {cat.map((categoriey) => (
              <img
                key={categoriey._id}
                src={categoriey.image}
